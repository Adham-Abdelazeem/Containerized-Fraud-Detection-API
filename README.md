...
2. **Model Training & Registry (MLflow)** — A logistic regression model trained on highly imbalanced credit card fraud data. Training runs log hyperparameters and evaluation metrics (accuracy, precision, recall, F1), and register the final model artifact to be automatically pulled by the serving layer.
3. **Production-Ready REST API** — A FastAPI endpoint (`/predict`) that dynamically fetches features and returns a fraud probability score. It includes `/health` (liveness) and `/ready` (readiness) probes for safe deployment in container orchestration systems (e.g., Kubernetes or ECS).
4. **Hardened Docker Environment** — The application is packaged using security best practices: a multi-stage build to reduce image size, and execution as a restricted, non-root user (`appuser`) to minimize attack surface.
...
Run the training script. This pulls historical features from Feast, trains the logistic regression model, and registers it in the local MLflow registry:
...
*(Returns `{"status": "ready"}` once MLflow and Feast are fully loaded into memory.)*
